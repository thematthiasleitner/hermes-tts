...
export default class NoteTtsAudioPlugin extends Plugin {
  ...
  private async synthesizeWithGemini(
    ...
    try {
      ...
      const shouldChunk =
        this.isGeminiTextInsteadOfAudioError(mapped.message) ||
        this.isGeminiMissingAudioError(mapped.message);
  ...
  private async synthesizeWithGeminiChunked(
    text: string,
    ai: GoogleGenAI,
    provider: GeminiProvider,
    instructions: string,
    ...
    progressNotice?.setMessage("Synthesizing in segments via Gemini…");
  ...
  private isGeminiMissingAudioError(message: string): boolean {
  ...
  private splitTextAtParagraphs(text: string, maxChunkSize: number): string[] {
    const cleaned = text.trim();
    if (!cleaned) return [];
...
class NoteTtsAudioSettingTab extends PluginSettingTab {
  ...
  private displayOpenAiSettings(containerEl: HTMLElement): void {
    new Setting(containerEl)
      .setName("API key")
      .setDesc("Get a key from platform.openai.com.")
      .addText((text) => {
  ...
  private displayGeminiSettings(containerEl: HTMLElement): void {
    new Setting(containerEl)
      ...
      .setDesc("Get a key from aistudio.google.com.")
      .addText((text) => {
        text.inputEl.type = "password";
        return text
          .setPlaceholder("API key")
  ...
  private displayGoogleCloudSettings(containerEl: HTMLElement): void {
    new Setting(containerEl)
      .setName("API key")
      .setDesc("Enable the cloud text-to-speech API and add an API key.")
      .addText((text) => {
        text.inputEl.type = "password";
        return text
          .setPlaceholder("API key")
  ...
  private displayElevenLabsSettings(containerEl: HTMLElement): void {
    new Setting(containerEl)
      .setName("API key")
      .setDesc("Get a key from your account at elevenlabs.io.")
      .addText((text) => {
        text.inputEl.type = "password";
        return text
          .setPlaceholder("API key")
    ...
    new Setting(containerEl)
      .setName("Model ID")
      .setDesc("Voice model identifier, such as eleven_multilingual_v2.")
      .addText((text) =>
  ...
  private displayOpenAiCompatibleSettings(containerEl: HTMLElement): void {
    new Setting(containerEl)
      .setName("API key")
      .setDesc("Bearer token for your endpoint.")
      .addText((text) => {
        text.inputEl.type = "password";
        return text
    ...
    new Setting(containerEl)
      ...
      .setDesc("Base URL for your endpoint.")
      .addText((text) =>
